...
<div class="mb-4 rounded bg-white p-4 shadow"><p><b>Mahasiswa:</b> {{ $guidance->student->name }}</p><p><b>Jenis:</b> {{ ucfirst($guidance->type) }} · {{ $guidance->chapter }}</p><p class="mt-3 whitespace-pre-line">{{ $guidance->student_note }}</p>@if($guidance->file_path)<a class="text-indigo-700" target="_blank" href="{{ route('guidances.file.view', $guidance) }}">Lihat file mahasiswa</a>@endif</div>
