...
<label class="mb-2 block font-semibold">Dosen Pembimbing</label><select name="supervisor_id" class="mb-4 w-full rounded border p-2"><option value="">- Pilih dosen -</option>@foreach($dosens as $d)<option value="{{ $d->id }}" @selected(old('supervisor_id',$title->supervisor_id)==$d->id)>{{ $d->name }}</option>@endforeach</select>
